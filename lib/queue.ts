...
const workersRegistry: Record<string, (data: any, progressCallback: (pct: number) => void) => Promise<any>> = {
  /**
   * Heavy process: Generates the daily delivery roster, mapping coordinates and optimization
   */
  "GENERATE_DELIVERY_ROSTER": async (data: { date: string }, progressCallback) => {
    progressCallback(10);
    // Fetch all scheduled deliveries for that date
    ...
    progressCallback(30);
    // Fetch customer details (to get addresses and locations)
    const users = await getUsers();
    
    progressCallback(50);
    // Geocode addresses and group them for dispatchers
    const roster = [];
    for (let i = 0; i < activeDrops.length; i++) {
      ...
      // Update progress relative to geocoding loop
      const loopPct = 50 + Math.floor((i / activeDrops.length) * 40);
      progressCallback(loopPct);
    }

    progressCallback(90);
    // Log the generated roster to a static cache/file in .gemini dir
...
/**
 * Worker loop that processes jobs in the background sequentially
 */
...
export const backgroundQueue = {
  /**
   * Adds a job to the background queue and runs worker in background
   */
  async addJob(name: string, data: any): Promise<Job> {
    ...
    // Asynchronously run worker so HTTP request returns instantly!
    setImmediate(() => {
